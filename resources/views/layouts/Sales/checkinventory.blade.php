        ...
        <thead class="bg-secondary ">
            ...
            @forelse($products as $product)   
                    <tr class="text-black font-normal odd:bg-[#CAD9FF]">
                        ...
                        <td class="px-6 py-2">{{ $product->stock }}
                        <td class="px-6 py-2">
                            @if($product->stock > 0)
                                Active  
                            @else 
        ...
        <div class="py-3 flex justify-center items-center gap-4 h-fit fade-in">
                ...
                <div class="w-auto px-2 h-fit">
                    <span class="text-sm text-gray-700 dark:text-gray-400">
                        Showing <span class="font-semibold text-gray-900">{{ $products->firstItem() }}</span> to <span class="font-semibold text-gray-900">{{ $products->lastItem() }}</span> of <span class="font-semibold text-gray-900 dark:text-white">{{ $products->total() }}</span> Entries
                    </span>
                </div>
