...
<div class="w-full h-full p-5">
        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
            ...
            <li aria-current="page">
                ...
                <svg class="rtl:rotate-180  w-3 h-3 mx-1 text-gray-400" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 6 10">
                    ...
                    <span class="ms-1 text-lg font-medium text-secondary md:ms-2">Deductions</span>
                </div>
            </li>
        </ol>
    </nav>
    ...
    @foreach($deductions as $deduction)
        <form action="{{ route('deduction.update', ['deduction_id' => $deduction->id]) }}" method="post" id="{{$deduction->id}}" tabindex="-1" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
            @csrf
            @method('PUT')
            <div  class="relative p-4 w-full max-w-md max-h-full">
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    <button type="button" class="absolute top-3 end-2.5 text-gray-400 bg-transparent hover:bg-gray-200 transition-colors duration-200 ease-in-out hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-hide="{{ $deduction->id }}">
                    ...
                    <div class="p-4 md:p-5 text-center">
                        ...
                        <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">Update {{ $deduction->description }}</h3>
                        ...
                        <div class="w-full my-3 text-left">
                            ...
                            <input type="text" name="description" autocomplete="off" value="{{ $deduction->description }}" id="description" placeholder="Description" class="w-auto outline-none focus:ring-1 focus:border-none focus:ring-secondary border border-gray-500 placeholder:text-gray-700 rounded p-2">
                        </div>
                        <div class="w-full my-3 text-left">
                            @error('amount')
                                <p class="text-xs text-red-600 font-semibold">{{ $message }}</p>
                            ...
                            <input type="number" name="amount" id="amount" value="{{ $deduction->amount }}" autocomplete="off" placeholder="Amount" class="w-auto outline-none focus:ring-1 focus:ring-secondary focus:border-none border border-gray-500 placeholder:text-gray-700 rounded p-2">
                        </div>
                        <button data-modal-hide="{{$deduction->id}}" type="button" class="py-2.5 transition-colors duration-200 ease-in-out px-5 ms-3 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 focus:z-10 focus:ring-4 focus:ring-gray-100 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700">
                            Cancel
                        </button>
    ...
    <div class="w-full p-4 h-fit bg-white rounded">
        <form action="{{ route('deduction.store') }}" method="post">
            @csrf
            <h3 class="mb-3 text-lg font-normal text-gray-500 dark:text-gray-400">Create new deduction</h3>
            <div class="w-full my-3 text-left">
                @error('description')
                    <p class="text-xs text-red-600 font-semibold">{{ $message }}</p>
                @enderror
    ...
    <table id="default-applicant-table" class="fade-in font-semibold text-md table-auto border text-white w-fit shadow">
        <caption class="text-gray-800 text-center">Deductions</caption>
        ...
        <tbody id="table-applicant-body" class="bg-white text-left rounded-b-lg">
            @forelse($deductions as $deduction)
                    <tr class="text-black font-normal odd:bg-[#CAD9FF]">
                        ...
                        <td class="px-1 py-2 text-center">&#8369; {{ $deduction->amount }}</td>
                        <td class="px-3 py-2 font-bold text-center">
                            <button data-modal-target="{{$deduction->id}}" data-modal-toggle="{{$deduction->id}}" class="block m-auto text-white bg-amber-500 hover:bg-amber-600 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm p-3 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" type="button">
