...
<div class="top-0 sticky left-0 flex flex-col items-start w-full justify-between overflow-y-hidden">
    ...
    <form action="{{ route('logout') }}" method="post" class="flex items-center justify-between w-full h-fit">
        @csrf
        <button class="flex w-full p-2 items-center rounded justify-between bg-transparent text-black hover:text-white hover:bg-red-600 transition-colors duration-200 ease-in-out">
            Logout
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="w-6 h-6" viewBox="0 0 16 16">
